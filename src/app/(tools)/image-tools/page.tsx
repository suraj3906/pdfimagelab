import Link from "next/link";
import { ImageIcon, Minimize, Maximize, FileImage, ArrowRight } from "lucide-react";

export default function ImageToolsCategoryPage() {
  const tools = [
    {
      title: "Compress Image",
      description: "Reduce image file size while maintaining quality.",
      href: "/image-tools/compress-image",
      icon: <Minimize className="h-6 w-6" />,
      color: "text-blue-600 bg-blue-100 dark:bg-blue-900/20 dark:text-blue-400",
    },
    {
      title: "Resize Image",
      description: "Change the dimensions of your images easily.",
      href: "/image-tools/resize-image",
      icon: <Maximize className="h-6 w-6" />,
      color: "text-green-600 bg-green-100 dark:bg-green-900/20 dark:text-green-400",
    },
    {
      title: "Convert Image",
      description: "Convert images between JPG, PNG, WEBP, and more.",
      href: "/image-tools/convert-image",
      icon: <FileImage className="h-6 w-6" />,
      color: "text-purple-600 bg-purple-100 dark:bg-purple-900/20 dark:text-purple-400",
    },
  ];

  return (
    <div className="container mx-auto px-4 py-12 max-w-6xl">
      <div className="flex flex-col items-center text-center space-y-4 mb-12">
        <div className="inline-flex items-center justify-center p-3 bg-blue-100 text-blue-600 rounded-full mb-2 dark:bg-blue-900/20 dark:text-blue-400">
          <ImageIcon className="h-8 w-8" />
        </div>
        <h1 className="text-4xl font-bold tracking-tight">Image Tools</h1>
        <p className="text-xl text-muted-foreground max-w-[700px]">
          Powerful image processing tools that run securely in your browser. Fast, free, and private.
        </p>
      </div>

      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {tools.map((tool) => (
          <Link
            key={tool.title}
            href={tool.href}
            className="group flex flex-col justify-between p-6 bg-card border rounded-xl hover:shadow-md transition-all hover:border-primary/50"
          >
            <div>
              <div className={`inline-flex p-3 rounded-lg mb-4 ${tool.color}`}>
                {tool.icon}
              </div>
              <h2 className="text-xl font-bold mb-2 group-hover:text-primary transition-colors">
                {tool.title}
              </h2>
              <p className="text-muted-foreground">{tool.description}</p>
            </div>
            <div className="flex items-center text-sm font-medium text-primary mt-6 opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-[-10px] group-hover:translate-x-0">
              Use Tool <ArrowRight className="ml-1 h-4 w-4" />
            </div>
          </Link>
        ))}
      </div>
    </div>
  );
}
